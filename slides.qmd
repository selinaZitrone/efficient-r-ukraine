...
- Collection of packages for data analysis (I/O, manipulation, visualization, modelling, ...)
- Consistent syntax, works well with pipes (`|>`, `%>%`)
- Easy and intuitive to learn
- Often fast enough but can be slower than alternatives
- Main data structure `tbl_df` but also databases and data.tables as backend possible
...
df <- data.frame(x = 1:100000, y = 1:100000, z = 1:100000)
...
ghg_emsDT <- setDT(ghg_ems) # to data table
arrow_tbl <- arrow_table(ghg_ems) # to arrow table
```
...
summarize_dt <- function() {
  ghg_emsDT[, mean(Electricity, na.rm = TRUE), by = Country]
}
